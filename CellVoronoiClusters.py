...
def switch_cluster(event):
    global cluster_index
    global united_regions
    global height
    global width
    global grans
    if event.key == 'right':
        if cluster_index == (len(set(labels))-1):
            cluster_index = 0
        else:
            cluster_index += 1
    elif event.key == 'left':
        if cluster_index == 0:
            cluster_index = (len(set(labels))-1)
        else:
            cluster_index -= 1
    plt.clf()
    print("[++] Loading cluster no. {index}".format(index=cluster_index+1))
    mask = united_regions.copy()
    for i in range(height):
        for j in range(width):
            if united_regions[i][j] == cluster_index+1:
                mask[i][j] = 1
            else:
                mask[i][j] = 0
    mask = np.array(mask, dtype=np.uint8)
    masked_image = np.stack((mask*grans[:,:,0], mask*grans[:,:,1], mask*grans[:,:,2]), axis=-1)
    plt.imshow(masked_image)
    plt.axis('off')
    fig.tight_layout()
    fig.canvas.manager.set_window_title("Cluster subplot no. {i}".format(i=cluster_index+1))
    plt.draw()

print("[WELCOME] to Cell-Voronoi-Clusters.py\nPlease use only png images and follow instructions.")
...
#UP UNTIL HERE EVERYTHING IS LIKE CELL VORONOI
regions = np.zeros_like(gray_grans[:,:])
height = grans.shape[0]
width = grans.shape[1]
print("[+] Manually computing Voronoi regions.")
for i in tqdm(range(height)):
    for j in range(width):
        distances = [math.dist([j,i], k) for k in centroids]
        min_dist = min(distances)
        label = distances.index(min_dist) + 1
        regions[i][j] = label
print("[+] Visualizing overlapped Voronoi regions.")
print("[i] Adjacent regions might have the same color.")
labeled_grans = ski.color.label2rgb(regions, gray_grans)
vor = Voronoi(centroids)
fig, ax = plt.subplots(); fig.canvas.manager.set_window_title("Voronoi regions")
ax.imshow(labeled_grans)
...
print("[+] Caluclating region properties. (area, eccentricity, perimeter, pearson correlation coefficient and p-value)")
label_grans = ski.measure.label(regions)
regions_of_grans = ski.measure.regionprops(label_grans, intensity_image=grans)
areas = []; eccentricities = []; perimeters = []; pearson = []; pearson_p = []; entropies = []
for i in tqdm(regions_of_grans):
    areas.append(i.area)
    eccentricities.append(i.eccentricity)
    perimeters.append(i.perimeter)
    ...
    for j in range(height):
        for k in range(width):
            if mask[j][k] != to_keep:
                ...
                mask[j][k] = True
    p1, p2 = ski.measure.pearson_corr_coeff(grans_h[:,:,0], grans_e[:,:,1], mask=mask)
    pearson.append(p1)
    pearson_p.append(p2)
    #applying mask over original rgb image
    masked_grans = np.stack((mask*grans[:,:,0], mask*grans[:,:,1], mask*grans[:,:,2]), axis=-1)
    entropies.append(ski.measure.shannon_entropy(masked_grans))
features = list(zip(areas, eccentricities, perimeters, pearson, pearson_p, entropies))
features = [list(i) for i in features]
max_minSamples = int(len(features)/2) #at least half
print("[+] Clustering with OPTICS algorithm and min_samples parameter testing from 2 to {i} with step {j}.".format(i=max_minSamples, j=math.ceil((max_minSamples-2)/30)))
silhouette_scores = []
for i in tqdm(range(2, max_minSamples+1, math.ceil((max_minSamples-2)/30))):
    clustering = OPTICS(min_samples=i).fit(features)
    labels = clustering.labels_
    if len(set(labels)) < 2:
        silhouette_scores.append(-2)
    else:
        silhouette_scores.append(metrics.silhouette_score(features,labels))
fig, ax = plt.subplots(); fig.canvas.manager.set_window_title("Silhouette score for OPTICS algorithm (best=1)")
plt.plot(silhouette_scores)
fig.tight_layout()
plt.show()
max_score = max(silhouette_scores)
best_minSamples = 2+silhouette_scores.index(max_score)*(math.ceil((max_minSamples-2)/30))
print("[i] minimum samples to be considered a cluster is {i}".format(i=best_minSamples))

clustering = OPTICS(min_samples=best_minSamples).fit(features)
labels = clustering.labels_

print("[i] CLUSTER LABELS ARE:")
print(labels)
if -1 in labels:
    labels = [i+2 for i in labels]
else:
    labels = [i+1 for i in labels]
print("[i] {j} clusters have been found.".format(j=len(set(labels))))
uniting_dict = {}
for i in range(len(centroids)):
    region_value = i+1
    uniting_dict[region_value] = labels[i]
united_regions = np.zeros_like(regions[:,:])
for i in tqdm(range(height)):
    for j in range(width):
        old_value = regions[i][j]
        new_value = uniting_dict[old_value]
        united_regions[i][j] = new_value
print("[+] Calculated new regions, now visualizing.")
print("[i] Adjacent regions might have the same color.")
labeled_grans_united = ski.color.label2rgb(united_regions, gray_grans)
fig, ax = plt.subplots(); fig.canvas.manager.set_window_title("OPTICS clustered regions")
ax.imshow(labeled_grans_united)
plt.show()

fig, ax = plt.subplots()
if len(set(labels)) > 1:
    print("[!] {i} similar regions have been found. Use left and right arrow keys to cycle through them.".format(i=len(set(labels))))
    cluster_index = 0
    starting_mask = united_regions.copy()
    for i in range(height):
        for j in range(width):
            if united_regions[i][j] == 1:
                starting_mask[i][j] = 1
            else:
                starting_mask[i][j] = 0
    starting_mask = np.array(starting_mask, dtype=np.uint8)
    starting_masked_image = np.stack((starting_mask*grans[:,:,0], starting_mask*grans[:,:,1], starting_mask*grans[:,:,2]), axis=-1)
    ax.imshow(starting_masked_image)
    ax.axis('off')
    cid_cycle = fig.canvas.mpl_connect("key_press_event", switch_cluster)
    fig.canvas.manager.set_window_title("Cluster subplot no. {i}".format(i=cluster_index+1))#0indexing
    fig.tight_layout()
    plt.show()
    fig.canvas.mpl_disconnect(cid_cycle)
else:
    print("[!] All regions belong to the same cluster")
    ax.imshow(grans)
    ax.axis('off')
    fig.tight_layout()
    plt.show()
