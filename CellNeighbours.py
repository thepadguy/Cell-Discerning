...
def distance_kernel(height, width):
    kernel = np.zeros_like(np.arange(width*height).reshape((height,width)), dtype='float64')
    center_x = math.ceil(width/2)
    center_y = math.ceil(height/2)
    for i in range(height):
        for j in range(width):
            value = math.dist([j+1, i+1], [center_x, center_y])
            if value != 0:
                kernel[i][j] = 1/value
    return kernel

print("[WELCOME] to Cell-Neighbours.py\nPlease use only png images and follow instructions.")
...
print("[+] Visualizing contour polygons.")
...
fig, ax = plt.subplots(); fig.canvas.manager.set_window_title("Masked-out contour polygons")
ax.imshow(masked_image, cmap=plt.cm.gray)
plt.show()


print("[+] Height and width of image are: {h}, {w} pixels.".format(h=gray_grans.shape[0], w=gray_grans.shape[1]))
print("Kernel dimensions must be positive numbers, smaller than the height and the width of the image.")
print("An example 3x3 kernel is:\n"+str(distance_kernel(3,3)))

#creating a while-loop for trying different kernel sizes
flag = True
while flag:
    flag = False
    k_width = input("[INPUT] kernel's width and press Enter: ")
    k_height = input("[INPUT] kernel's height and press Enter: ")
    k_width = int(float(k_width)); k_height = int(float(k_height))
    kernel = distance_kernel(k_height, k_width)
    
    conved_image = convolve2d(masked_image, kernel, mode='same')
    #normalize conved image in range 0-1
    min_val = conved_image.min()
    max_val = conved_image.max()
    conved_image[:,:] = (conved_image[:,:] - min_val)/(max_val - min_val)
    fig, ax = plt.subplots()
    fig.canvas.manager.set_window_title("Contour neighbourhoods with {h}x{w} kernel".format(h=k_height, w=k_width))
    #ax.imshow(grans)
    masked_conved_image = np.stack(( (grans[:,:,0]*conved_image).astype('int'), (grans[:,:,1]*conved_image).astype('int'), (grans[:,:,2]*conved_image).astype('int')), axis=-1)  #alpha has 0..1 values
    ax.imshow(masked_conved_image)
    plt.show()
